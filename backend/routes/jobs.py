...
async def get_applied_applicants(current_user: dict = Depends(get_current_user)):
    ...
    # Step 3: Collect applicant emails
    applicant_emails = list(set(app["applicant_email"] for app in applications))

    print("Emails count:", len(applicant_emails))

    # Step 4: Get all applicants
    ...
    applicant_map = {app["email"]: app for app in applicants}

    # Step 5: Group applicants by job
    job_applicants = {job_id: [] for job_id in job_ids}

    for app in applications:
        email = app["applicant_email"]
        job_id = app["job_id"]

        if email in applicant_map:
            applicant = applicant_map[email].copy()
            applicant.pop("password", None)
            applicant.pop("resume", None)
            applicant["_id"] = str(applicant["_id"])

            job_applicants[job_id].append(applicant)
...
# resume download when employeer wants
from fastapi.responses import Response
from bson import ObjectId
import base64
...
async def download_resume(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{resume.get("filename", "resume")}"'
        }
    )
