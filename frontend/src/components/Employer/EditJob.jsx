...
export default function EditJob() {
  ...
  const fetchJob = async () => {
    try {
      const res = await API.get(`/edit-jobs/data/${id}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await API.put(`/edit-job/${id}`, form, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      {message && (
        <div className="fixed top-6 left-1/2 z-50 w-full px-4">
          <div className="fixed top-6 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-sm px-4">
            <div
              className={`px-6 py-4 rounded-lg shadow-lg text-center max-w-md w-full ${
                messageType === "success"
      ...
      <div className="min-h-screen bg-gradient-to-r from-indigo-900 to-purple-900 flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-xl p-8 rounded-2xl shadow-xl">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <InputField label="Job ID" type="number" name="jobId" value={form.jobId} onChange={handleChange} />
            ...
            <InputField label="Job Type" name="jobType" value={form.jobType} onChange={handleChange} />

            <InputField label="Required Skills" name="skills" value={form.skills} onChange={handleChange} />

            <InputField label="Salary" name="salary" value={form.salary} onChange={handleChange} />

            <InputField label="Experience" name="experience" value={form.experience} onChange={handleChange} placeholder="e.g. 2+ years" />
