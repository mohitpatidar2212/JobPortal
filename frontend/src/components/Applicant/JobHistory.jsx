...
export default function JobHistory() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await API.get("/applicant/jobs/applied", {
        headers: {
  ...
  const handleWithdraw = async (jobId) => {
    try {
      const token = localStorage.getItem("token");
      await API.delete(`/jobs/${jobId}/application/delete`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
              ...
              {history.map((job) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="mt-5 bg-red-500 text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-red-600 transition w-full md:w-auto"
                  >
          ...
          <div className="fixed inset-0 backdrop-blur-xs bg-opacity-40 flex items-center justify-center z-50 px-4">
            <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
              ...
              <button
                ...
                className="mt-5 bg-red-500 text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-red-600 transition w-full"
              >
                Withdraw Application
              </button>
            </div>
