...
export default function AdminDashboard() {
    ...
    const handleDeleteEmployer = async (id) => {
        ...
        try {
        const token = localStorage.getItem("token");
        await API.delete(`/admin/delete-employer/${id}`, {
            headers: {
            Authorization: `Bearer ${token}`,
            },
        });
    ...
    const handleDeleteApplicant = async (id) => {
        ...
        try {
        const token = localStorage.getItem("token");
        await API.delete(`/admin/delete-applicant/${id}`, {
            headers: {
            Authorization: `Bearer ${token}`,
            },
        });
    ...
    return (
        <div className="max-w-5xl mx-auto py-10 px-4 min-h-[70vh]">
            ...
            <div className="flex flex-wrap gap-4 mb-6">
                ...
                className="px-4 py-2 bg-indigo-600 text-white rounded max-w-[250px] w-full sm:w-auto"
                >
                View Employers
                </button>
                ...
                <button
                onClick={fetchJobs}
                className="px-4 py-2 bg-green-600 text-white rounded max-w-[250px] w-full sm:w-auto"
                >
                View Jobs
                </button>
            </div>
            ...
            {view === "employers" && (
                ...
                {employers.map((e) => (
                    ...
                    <button
                        ...
                        className="px-3 py-1 bg-red-600 text-white text-sm rounded max-w-[150px] w-full sm:w-auto"
                    >
                        Delete
                    </button>
                    </div>
